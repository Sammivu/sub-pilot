...
public interface PlanRepository extends JpaRepository<Plan, String> {
    ...
    Page<Plan> findByMerchantIdAndStatus(String merchantId, String status, Pageable pageable);
    ...
    Optional<Plan> findBySlugAndStatus(String slug, String status);
    ...
    @org.springframework.data.jpa.repository.Query("""
        SELECT p FROM Plan p JOIN co.subpilot.merchant.Merchant m ON p.merchantId = m.id
        WHERE m.slug = :merchantSlug AND p.slug = :planSlug AND p.status = 'published'
    """)
